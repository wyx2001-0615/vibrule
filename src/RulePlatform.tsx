  ...
  return <main className="flow-app">
    ...
    <div className="flow-body">
      <aside className="node-palette">
        <div className="palette-head"><h1>{sidebarMode==="nodes"?"节点工具箱":"历史数据"}</h1><p>{sidebarMode==="nodes"?"最小单元节点，可重复组合":"拖到画布或已有波形节点"}</p></div>
        ...
        {sidebarMode==="nodes"&&<><div className="source-import-tip"><span>i</span><div><strong>默认画布已搭建完整判别实例</strong><small>拖入相同节点可继续组合其他诊断规则</small></div></div><div className="quick-params"><label>新波形采样频率<input value={defaultFs} type="number" onChange={(event)=>setDefaultFs(Number(event.target.value)||1)}/><em>Hz</em></label><label>设备转速备注<input value={rpm} type="number" onChange={(event)=>setRpm(Number(event.target.value)||0)}/><em>rpm</em></label></div><div className="palette-scroll classified-palette">{palette.map((group,index)=><details key={group.group} open={index<4}><summary><span>{group.group}</span><b>{group.items.length}</b></summary>{group.items.map((item)=><div className="palette-node" key={`${item.kind}-${item.title}`} draggable onDragStart={(event)=>{event.dataTransfer.effectAllowed="copy";event.dataTransfer.setData("application/x-vibrule-node",JSON.stringify(item));}}><span>{item.icon}</span><div><strong>{item.title}</strong><small>{item.desc}</small></div><b>⠿</b></div>)}</details>)}</div></>}
