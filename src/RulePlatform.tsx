...
const MAX_SCALE = 1.8;
const DEFAULT_SOURCE_FS = 10240;
const DEFAULT_SOURCE_RPM = 1500;
...
export default function RulePlatform() {
  ...
  function sourceFs(id:string){return Math.max(1,nodes.find((item)=>item.id===id)?.samplingFrequency??DEFAULT_SOURCE_FS);}
  function updateSourceConfig(id:string,change:Pick<FlowNode,"samplingFrequency"|"rpm">){
    updateNode(id,change);
    if(change.samplingFrequency!==undefined)setSourceSignals((current)=>current[id]?{...current,[id]:{...current[id],fs:Math.max(1,change.samplingFrequency??DEFAULT_SOURCE_FS),revision:Date.now()}}:current);
  }
  ...
  function loadExampleData(){
    const now=Date.now();
    const normalFs=sourceFs("normal"),faultFs=sourceFs("fault");
    setSourceSignals((current)=>({...current,normal:{samples:generateDemoSignal(normalFs,"normal"),fs:normalFs,fileName:"正常工况演示.txt",revision:now},fault:{samples:generateDemoSignal(faultFs,"fault"),fs:faultFs,fileName:"外圈故障演示.txt",revision:now+1}}));
  ...
  async function loadFile(sourceId:string,file?:File){if(!file)return;try{const samples=parseTextSignal(await file.text()).slice(0,65536),fs=sourceFs(sourceId);if(samples.length<64)throw new Error("too short");setSourceSignals((current)=>({...current,[sourceId]:{samples,fs,fileName:file.name,revision:Date.now()}}));notify(`${file.name}：已载入 ${samples.length.toLocaleString()} 点，采样频率 ${fs.toLocaleString()} Hz`);}catch{notify("无法识别文件，请使用单列或“时间,幅值”格式");}}
  function loadDemo(sourceId:string){const current=nodes.find((item)=>item.id===sourceId),role=current?.title.includes("正常")?"normal":"fault",fs=sourceFs(sourceId);setSourceSignals((values)=>({...values,[sourceId]:{samples:generateDemoSignal(fs,role),fs,fileName:role==="normal"?"正常工况演示.txt":"外圈故障演示.txt",revision:Date.now()}}));notify(`${current?.title??"波形"}演示数据已载入`);}
  ...
  async function importPendingHistory(){
    ...
    for(const file of pendingHistoryFiles){try{const values=parseTextSignal(await file.text());if(values.length<64)throw new Error("too short");await saveHistory(file,{sampleCount:Math.round(historySampleCount),samplingFrequency:historyMaxFrequency*2,rpm:DEFAULT_SOURCE_RPM,bpfo:0});success++;}catch{failed++;}}
  ...
  async function loadHistoryIntoNode(record:HistoryRecord,sourceId:string){notify(`正在读取历史数据：${record.file_name}`);try{const samples=parseTextSignal(await downloadHistory(record)).slice(0,Math.min(65536,record.sample_count||65536)),fs=record.sampling_frequency||DEFAULT_SOURCE_FS;if(samples.length<64)throw new Error("too short");updateNode(sourceId,{samplingFrequency:fs,rpm:record.rpm||DEFAULT_SOURCE_RPM});setSourceSignals((current)=>({...current,[sourceId]:{samples,fs,fileName:record.file_name,revision:Date.now(),historyId:record.id}}));notify(`${record.file_name} 已装载，点击运行后计算`);}catch{notify("历史波形读取失败，请检查云端文件是否存在");}}
  ...
  function renderNodeBody(node:FlowNode){
    ...
    if(node.kind==="source")return <div className="node-body source-node-body"><div className="source-wave-preview"><TinyWave samples={source?.samples??[]}/><div><strong>{source?.fileName??"未导入波形"}</strong><small>{source?`${source.samples.length.toLocaleString()} 点 · ${source.fs.toLocaleString()} Hz`:"导入文件或接收历史数据"}</small></div></div><div className="source-node-config"><label>采样频率<div><input type="number" min="1" value={node.samplingFrequency??DEFAULT_SOURCE_FS} onChange={(event)=>updateSourceConfig(node.id,{samplingFrequency:Number(event.target.value)||1})}/><em>Hz</em></div></label><label>设备转速备注<div><input type="number" min="0" value={node.rpm??DEFAULT_SOURCE_RPM} onChange={(event)=>updateSourceConfig(node.id,{rpm:Number(event.target.value)||0})}/><em>rpm</em></div></label></div><div className="source-node-actions"><label><input type="file" accept=".txt,.csv" onChange={(event)=>{void loadFile(node.id,event.target.files?.[0]);event.currentTarget.value="";}}/><span>＋ 导入波形</span></label><button onClick={()=>loadDemo(node.id)}>演示数据</button></div>{source?.historyId&&<div className="history-linked">☁ 已连接历史数据</div>}</div>;
  ...
  return <main className="flow-app">
    ...
    <div className={`flow-body ${sidebarCollapsed?"sidebar-collapsed":""}`}>
      <aside className="node-palette">
        ...
        <div className="palette-head"><h1>{sidebarMode==="nodes"?"节点工具箱":"历史数据"}</h1><p>{sidebarMode==="nodes"?"从基础节点开始，后续按需逐步加入":"拖到画布或已有波形节点"}</p></div>
        ...
        {sidebarMode==="nodes"&&<div className="palette-scroll classified-palette">{palette.map((group,index)=><details key={group.group} open={index===0}><summary><span>{group.group}</span><b>{group.items.length}</b></summary>{group.items.map((item)=><div className="palette-node" key={`${item.kind}-${item.title}`} draggable onDragStart={(event)=>{event.dataTransfer.effectAllowed="copy";event.dataTransfer.setData("application/x-vibrule-node",JSON.stringify(item));}}><span>{item.icon}</span><div><strong>{item.title}</strong><small>{item.desc}</small></div><b>⠿</b></div>)}</details>)}</div>}
